...
from a1.funções.modelos import criar_targets_binarios, salvar_importancias, treinar_modelo_xgboost, gerar_pdp, \
    clean_feature_names
...
volumetria_safra.to_excel('volumetria_por_safra_18_a_30.xlsx', sheet_name='Safra', index=True)
...
for feature in top_5_features_d:
    if feature not in X.columns or X[feature].isnull().all():
        print(f"Feature '{feature}' has no valid data for PDP generation.")
...
for feature in top_5_features_n:
    if feature not in X.columns or X[feature].isnull().all():
        print(f"Feature '{feature}' has no valid data for PDP generation.")
